...
외부 API를 쓰지 않고 교내 GPU 장비의 로컬 모델로 처리한다.
배경과 설계 원칙은 [PROJECT_BRIEF.md](PROJECT_BRIEF.md).
...
src/zzaimy/
  ...
  eval/       평가 하네스
configs/      서빙·학습 설정
scripts/      실행 진입점(번호가 파이프라인 순서)
docs/         설계 문서 · 결정 기록(decisions/)
...
| ZZAIMY-Writer | Qwen3.8-27B | 섹션 생성 | 베이스라인 대기 |
...
검색·추출은 색인과 파서 모델이 있어야 동작한다. 없으면 해당 기능만 꺼진 채 뜬다.
