...
#
# 운영에서는 학습본 리랭커를 쓰고 `.env.local` 의 ZZAIMY_RERANK_MIN 으로 그 모델의 값을 준다.
# 학습본(zzaimy-rerank-v1)은 점수가 또렷해 0.005 다: 임베딩 v2 로 후보가 바뀐 뒤 다시 재도
# 같은 값이었다(정답 1위 하위5% 0.189·중앙 1.000, 무관 질의 19건 최대 0.000, 정답 오판 1/150).
# 아래 기본값은 베이스 모델(bge-reranker-v2-m3)을 VM/서비스로 쓸 때의 값이다.
RERANK_MIN = 0.271
