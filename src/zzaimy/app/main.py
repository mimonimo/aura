...
- 【이번 주 한 일】은 아래 [기준 틀]의 큰 축(플랫폼·모델(LLM)·데이터 같은 것) 단위로 3~5개. 큰 항목은 "무엇 — 결과" 한 줄
  ...
  세부 줄은 항목당 최대 1줄. 과정 서술(원인을 파악해 무효 처리했다, 보고 방식을 개선했다, 규칙을 고쳤다 같은 말)은
  쓰지 않는다 — 읽는 사람은 결과만 안다. 수치는 항목당 하나까지, 그것도 뜻이 바로 보이는 것만
  (예: "검색 정확도 75%" 는 되고 "R@1 0.750"·"MRR" 은 안 된다).
- 【다음 주 계획】은 3~4개, 각 한 줄, 목표 단위로("실물 문서 반입과 파일럿", "파인튜닝 착수", "규정 RAG 정식 구축"처럼).
  [다음에 할 일] 원자료에서만 고른다.
- 【이슈/건의사항】은 실제 막힌 것·요청할 것만. 없으면 "없음".
- 피드백에 질문이 있으면(예: "왜 이 모델인가", "표·이미지는 어떻게 처리하나") 【이번 주 한 일】 맨 앞에
  "피드백 답변" 항목을 두고 질문마다 두세 줄로 원자료에 근거해 답한다 — 모델 선정 이유는 [기준 틀]의 근거(멀티모달·학습 경로·
  벤치마크 수치와 그 한계)를 그대로 쓴다. 라이선스·명명 같은 말은 쓰지 않는다. 질문을 이슈나 건의사항으로 되돌리지 않는다.
