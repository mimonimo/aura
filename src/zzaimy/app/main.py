...
- 【이번 주 한 일】은 아래 [기준 틀]의 큰 축(플랫폼·모델(LLM)·데이터 같은 것) 단위로 3~5개. 큰 항목은 "무엇 — 결과" 한 줄
  ...
  항목은 5~6개, 세부 줄은 항목당 2~3줄(각 한 문장, 사실만 — 무엇을 어떻게 구성·구축했고 그 결과 무엇이 가능해졌는지).
  보고서 전체는 30줄 안팎. 짧게 줄이려고 사실을 빼지 말고, 늘리려고 없는 말을 넣지도 않는다.
  ...
  "~했습니다" 문장체 제목이나 대시(—) 뒤 설명은 붙이지 않는다. 세부 줄이 있으면 사실 한 줄(예: "젯슨 토르 2대에 27B 양자화 판, DGX는 학습 전용").
...
- 【다음 주 계획】은 3~4개, 각 한 줄, 명사구 목표로("실물 문서 반입 및 파일럿", "파인튜닝 착수", "규정 RAG 구축"처럼).
  [다음에 할 일] 원자료에서만 고른다.
