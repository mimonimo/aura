...
def _build_recognizers() -> list[PatternRecognizer]:
    return [
        ...
        _pattern_recognizer(
            "KR_NAME",
            "kr_name_list",
            # 참석자·위원 명단처럼 세 글자 이름이 쉼표·가운뎃점으로 셋 이상 이어진 것(실측 2026-09-24 판독 표 "박재훈, 김장환, 박민규, …").
            # 두 글자 낱말 나열(서울·인천·경기, 경제, 사회, 문화)은 명단이 아니다(공개 공고 62곳 오탐) — 세 글자만, 그리고 앞에
            # 명단임을 알리는 낱말(참석·위원·교수·담당·성명 …)이 가까이 있을 때만. 한 덩어리로 가린다
            r"(?<=(?:참석자|참석|위원장|위원|명단|교수|교원|담당자|담당|연구원|연구진|성명|이름|팀원|팀장|구성원|참여자|출석)"
            r"[^가-힣]{0,6}(?:[가-힣]{1,3}\*{1,3}[^가-힣]{0,4}){0,3})"
            r"(?<![가-힣])" + _NAME_GUARD + r"[가-힣]{3}(?:\s*[,、·･]\s*" + _NAME_GUARD + r"[가-힣]{3}){2,}(?![가-힣])",
            0.55,
        ),
        _pattern_recognizer(
            "KR_NAME",
