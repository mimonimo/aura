...
| 문서함 전체 재반입(9/20 밤) | 진행 | 사용자 지시로 문서함을 비우고 원본에서 126건을 다시 올림(`scripts/115_reingest_all.py`). 반입 단계에서 문서 이름이 본문 제목으로 바뀐다(`test.pdf` 가 여럿이어도 구분됨). 끝나면 색인·그래프 재구성과 배치 대조(Codex C-42) |
