...
| 문서 묶음 접수·구글 열람 (9/24, ADR-0032) | **실사용 확인(운영 프로젝트 3)** | `POST /projects/bundle`(기준/접수 자동 분리·이름 자동), 대화 첨부 = 문서, `/doc/{id}/view`·`/api/doc/{id}/google`(xlsx→시트·pptx→슬라이드·docx→독스·hwp→복원 docx→독스·pdf 미리보기, 문서당 한 번, 폴더 `ZZAIMY/<연도>/<프로젝트>/첨부/`), 문서함 `/api/chat/{sid}/documents`. 남은 것: 실제 세트로 재시험, 아스트라 C-80 |
