...
- **젯슨 토르** `thor-03@211.170.162.121`·`thor-02@211.170.162.120`, SSH 포트 8022(맥 키 등록됨, sudo 는 비밀번호 필요).
  ...
  **9/21 확정 분담**: 토르 02 = 대화·초안(사람이 기다리는 일), 토르 03 = 반입 검토·이미지 판독(배치) + 임베딩·리랭커.
  같은 27B(`zzaimy-writer`)를 두 대에 각각 올린다. 지금은 토르 02 bf16 한 대(:8001)가 셋을 다 맡고 있고 — 실측 동시 3요청
  생성 합계 14 토큰/초 — NVIDIA 공식 NVFP4 판(14GB)을 두 대에 받는 중이다. 서빙은 `scripts/126_serve_writer.sh`.
...
| 문서함 전체 재반입(9/20~21) | 반입 끝·색인 진행 | 문서함을 비우고 원본에서 다시 올렸다 — 산학협력단 규정 8 · 교내 내려받기 113 · 국고사업 공개 문서 68 · 외부 공고 5 = **문서 194건 · 기준 조각 3,757개**. 반입 단계에서 문서 이름이 본문 제목으로 바뀐다(`test.pdf` 가 여럿이어도 구분됨). 남은 실패는 내려받기가 막힌 파일 1건(HTML 오류 쪽이 .pdf 로 저장됨)과 20MB 스캔본 1건. 다음: 색인 재계산(`scripts/96` · 운영 모델 zzaimy-embed-v2) → 개체 추출(70) → 반입 점검(84) → 검색 정확도 재측정 |
