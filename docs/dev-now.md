## 2026-09-09 — 작업 기준=VM 확정 · 재조정·배포 완료

- 이 세션 중 맥 사본에서 작업해 VM과 갈라졌던 것을 3-way 병합으로 무손실 통합
  (VM 백업 `c7833e5` · 맥 `041cd60` · 병합 `e986dea`). **앞으로 모든 작업은 VM에서 직접** 한다.
- 배포: 앱 재시작(:443) · 라우트 검증. 국고 코퍼스(`corpus_pilot.db`, 1846조각) VM 적재 →
  `/dev/corpus` 라이브. 서술형 청커(`chunk_document`) 인제스트 연결.
- 남음: 코퍼스 dense 재색인(`66_reindex.sh`) · exe 실빌드(Windows 워커) ·
  명령콘솔/질의제출 자연어화(LLM) · GitHub 푸시(보류).
