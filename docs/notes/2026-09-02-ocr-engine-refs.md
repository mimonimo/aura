...
| dots.ocr 서빙 스모크 (09-02) | Spark 실측 | 성공 | vLLM nightly 도커(검증 이미지 재사용)로 8001 서빙 — gpu-mem 0.12(≈14.4GiB)면 Qwen3.5(0.85)와 병행 가능. 레이아웃 JSON(bbox·카테고리·텍스트) 정상, 표는 HTML로, 한국어 합성 표본 인식 깨짐 없음 |
| dots.ocr 실물 대결 (09-03, 문서 173 p3/5/7) | MinerU pipeline과 동일 페이지 대조 | **보류 확정 — MinerU 유지** | 고유명사 "희망사다리Ⅱ유형": MinerU 표에서 정확, dots는 "예만사다리"·"희망사다리교" 2회 오독. dots에서 영어 환각("affected practice")·"장학생→장식물"·"주chl등록번호" 등 그럴듯한 지어내기 발생 — 행정문서 최위험 유형. 영역 처리(스크린샷 통짜/내부 OCR)도 페이지마다 비일관. 단 dots가 스크린샷 내부를 더 잘 파는 페이지도 있어(p5), 커버리지 보조로의 재검토 여지는 남김. 자료: Spark /tmp/bakeoff/ |
