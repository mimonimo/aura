...
| dots.ocr 서빙 스모크 (09-02) | Spark 실측 | **성공** | vLLM nightly 도커(검증 이미지 재사용)로 8001 서빙 — gpu-mem 0.12(≈14.4GiB)면 Qwen3.5(0.85)와 병행 가능. 레이아웃 JSON(bbox·카테고리·텍스트) 정상, 표는 HTML로, 한국어 합성 표본 인식 깨짐 없음. 다음: 실물 페이지 bake-off |
